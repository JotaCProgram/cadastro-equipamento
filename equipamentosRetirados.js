...
function filtrarEquipamentosRetirados() {
    let tipoONT = document.getElementById('filtroONT').checked;
    let tipoONU = document.getElementById('filtroONU').checked;
    ...
    equipamentosRetirados.forEach(equipamento => {
        if ((tipoONT && equipamento.tipo === 'ONT') || (tipoONU && equipamento.tipo === 'ONU')) {
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.getElementById('goToJsonForExcel').addEventListener('click', function() {
        window.location.href = 'jsonforexcel.html'; // Atualize o caminho conforme necessário
    });
});
